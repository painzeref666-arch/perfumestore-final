...
export async function withTimeout<T>(promise: PromiseLike<T>, ms = 8000, label = 'Supabase request'): Promise<T> {
...
export function productToRow(product: ManagedProduct) {
  return {
    ...
    image: product.image,
    gallery_images: product.gallery_images || product.gallery || product.image_gallery || [],
...
export function rowToProduct(row: any): ManagedProduct {
  return {
    ...
    gallery: Array.isArray(row.gallery_images) ? row.gallery_images : Array.isArray(row.gallery) ? row.gallery : Array.isArray(row.image_gallery) ? row.image_gallery : [],
